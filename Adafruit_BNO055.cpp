...
imu::Vector<3> Adafruit_BNO055::getVector(adafruit_vector_type_t vector_type) {
  ...
  if (!readLen((adafruit_bno055_reg_t)vector_type, buffer, sizeof(buffer))) {
    xyz[0] = xyz[1] = xyz[2] = 0;
    return xyz;
  }
...
byte Adafruit_BNO055::read8(adafruit_bno055_reg_t reg) {
  ...
  if (i2c_dev->write_then_read(buffer, 1, buffer, 1, true) == false)
    return (byte)0;
...
bool Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, byte *buffer,
  ...
  return i2c_dev->write_then_read(reg_buf, 1, buffer, len, true);
}
